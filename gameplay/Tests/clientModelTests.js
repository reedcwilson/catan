...
test("Querry Functions", function()
{
  ...
  ok(client.canBuyRoad(client.getPlayers()[0]) == false, "Sam cannot buy because there isn't resources");
  ok(client.canBuyRoad(client.getPlayers()[2]) == false, "Pete cannot buy because it isn't his turn");
  client.getPlayers()[0].resources.brick = 1;
  client.getPlayers()[0].resources.wood = 1;
  client.getPlayers()[0].resources.sheep = 0;
  ok(client.canBuyRoad(client.getPlayers()[0]) == true, "Sam can buy because there are resources");
  ok(client.canBuySettlement(client.getPlayers()[0]) == false, "Sam cannot buy because there isn't resources");
  ok(client.canBuySettlement(client.getPlayers()[2]) == false, "Pete cannot buy because it isn't his turn");
  client.getPlayers()[0].resources.sheep = 1;
  client.getPlayers()[0].resources.wheat = 1;
  ok(client.canBuySettlement(client.getPlayers()[0]) == true, "Sam can buy beause there are resources");
  ...
  ok(client.canBuyCity(client.getPlayers()[0]) == false, "Sam cannot buy because there isn't resources");
  ok(client.canBuyCity(client.getPlayers()[2]) == false, "Pete cannot buy because it isn't his turn");
  client.getPlayers()[0].resources.wheat = 2;
  client.getPlayers()[0].resources.ore = 3;
  ok(client.canBuyCity(client.getPlayers()[0]) == true, "Sam can buy because there are resources");
});
